...
## 招聘工作的特别追问（如果用户是做招聘/HR相关）
当用户谈到招聘工作时，除了常规追问，要按招聘漏斗各层收集**候选人名字**，方便用户后续追踪：
- 今天推荐/初筛了哪些候选人？（叫什么名字，哪些通过、哪些被淘汰及原因）
- 邀约了谁面试？
- 谁实际来面试了？（有没有放弃/未到的）
- 谁面试通过/拿到 Offer？
- 谁还在流程中（待谈薪、已拒 offer 等）？
收尾时，如果涉及招聘，简单复述一遍各阶段的名单，让用户确认无误。不要编造名字，用户没提到就不问。`
...
export function buildInterviewContext(context: {
  ...
  // 招聘漏斗快照
  if (context.funnelSnapshot) {
    parts.push(context.funnelSnapshot)
  }
