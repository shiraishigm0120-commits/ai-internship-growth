...
export async function POST(req: Request) {
  ...
  if (internshipId) {
    ...
    if (internship) {
      ...
      // Load growth memory for long-term context
  ...
  if (!isReal) {
    ...
    const demoPrefix = isFirstMsg
      ? "💡 当前为**演示模式**（未配置 OpenAI API Key）。去 个人 → 个人设置 中添加你的 Key 即可解锁真实 AI 教练。\n\n---\n\n"
      : ""
    ...
    // After ~6 messages, suggest wrapping up
    ...
    const stream = new ReadableStream({
      start(controller) {
        const fullContent = demoPrefix + response
        // Stream word by word for a more natural feel
        const chunks = fullContent.match(/[\s\S]{1,3}/g) ?? [fullContent]
